...
class UserService {
    ...
    static async verifyEmail(email, locale) {
        ...
        if (prev) {
            ...
            console.log('Calling create method with email:', email, 'and verification code:', verificationCode);
            ...
            // Ensure email and verificationCode are defined
            if (!email || !verificationCode) {
                return { response: false, message: "Email or verification code is undefined.", data: null }
            }
            let result = await EmailVerifyModel.create({ email: email, verify_code: verificationCode })
            if (!result) {
                return { response: false, message: "An error was caused during the Email Verification Code generation.", data: null }
